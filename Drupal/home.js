import React from 'react';

const DrupalSection = () => {
  return (
    <section className="relative w-full  bg-black flex items-center justify-center p-4 min-h-screen">
      <div className="max-w-6xl mx-auto text-center space-y-8">
        <h1 className="text-6xl md:text-8xl font-light bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 bg-clip-text text-transparent">
        Drupal
        </h1>
        
        <p className="text-2xl md:text-3xl font-light text-gray-400 leading-relaxed">
        Discover AlgorithmX’s Drupal development services where creativity meets functionality to create unforgettable digital experiences and design captivating platforms with the goal of meeting customer needs and significantly increasing your ROI.
        </p>
      </div>
    </section>
  );
};

export default DrupalSection;